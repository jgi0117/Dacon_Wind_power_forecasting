"""Teacher-student RealMLP 학습·평가·제출 생성을 실행하는 진입점."""
...
MULTITASK_STRATEGY = (
    'privileged-12h-feature-target-teacher-to-direct-student-distillation'
)
    ...
    for model_name in config.models:
        ...
        model_schedule['student'] = int(
            model_metadata.get(
                ...
                model_metadata.get('best_iteration', config.max_epochs),
            )
        )
...
def _fit_final_models(
    ...
    if observed_rows.any() and y_joint.index[observed_rows].max() >= final_fit_cutoff:
        raise RuntimeError("최종 학습 정답에 예측기준시점 이후 행이 포함됐습니다.")

    for model_name in config.models:
        iterations = iteration_schedule.get(model_name) or None
        LOGGER.info(
            "최종 teacher-student 모델 학습 및 추론: %s / rows=%d / iterations=%s",
            model_name,
            len(X_joint),
            iterations,
        )
...
def _monthly_evaluation(
    ...
    return result[
        columns + [column for column in result if column not in columns]
    ]
...
def run_pipeline(config: PipelineConfig) -> pd.DataFrame:
    ...
    results.to_csv(
        config.output_dir / "evaluation_results.csv",
        index=False,
        encoding="utf-8",
    )
    _monthly_evaluation(
    ...
    pd.concat(
        validation_predictions,
        names=["model", "forecast_kst_dtm"],
    ).reset_index().to_csv(
        config.output_dir / "validation_predictions.csv",
        index=False,
        encoding="utf-8",
    )
    ...
    report_payload = {
        ...
        "split": {
            ...
            "iteration_selection_splits": iteration_splits,
